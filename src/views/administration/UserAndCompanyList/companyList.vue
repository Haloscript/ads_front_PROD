<template>
  <div class="content_tabs_table users_table">
    <table class="content_tabs_table_row">
      ...
      <tr v-for="company in COMPANYSLIST.data.shops" :key="company.id">
        <td>{{ company.title }}</td>
        <td>
          <a href="chat.html">
            <img
        ...
        <td style="text-align: center;">
          {{ moment(COMPANYSLIST.createdAt).format("DD.MM.YY") }}
        </td>
        <td style="text-align: center;">
          <router-link
            :to="{ path: '/administration/updatecompany/' + company.id }"
            type="button"
